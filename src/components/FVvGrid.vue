      ...
      <template v-slot:body="props">
        ...
        >
          <q-td class="q-pa-none" v-if="editable">
            <div
              class="q-pa-none fit flex flex-center text-center non-selectable"
            >
        ...
        <div class="col-12 col-md full-width">
          <div class="row q-gutter-sm text-h6 q-px-sm">
            ...
            <span
              ...
              v-if="nbResParking && String(nbResParking).trim() != ''"
              >{{ nbResParking }} réservations PARKING</span
            >
          </div>
          ...
          <div class="row q-gutter-sm q-px-sm q-pb-md justify-center">
            ...
            <q-btn
              label="Chronologie aujourd'hui"
              icon="timeline"
              no-caps
              @click="showTimeLine"
              class="text-subtitle2 text-bold text-primary bg-amber-3"
            ></q-btn>
          </div>
      ...
      <q-card style="width: 80%; max-width: 100%">
        ...
        <q-card-section class="q-pa-md" style="width: 100%; max-width: 100%">
          <f-vv-form-by-name
            ref="editForm"
            :nameForm="
              selectedRow.typeacces == 'PARKING' ? 'Demande Parking' : 'Accueil'
            "
...
function onChange(files) {
  ...
  reader.onload = (e) => {
    ...
    if (Array.isArray(dataHeader) && dataHeader.length > 0) {
      ...
      //this.selectedCols = [];
      //this.memColumns = [];
      console.log('Keys headers :', JSON.parse(JSON.stringify(dataHeader[0])));
...
function whereDateBetween(date1, date2) {
  return (item) =>
    date.isBetweenDates(
      ...
      new Date(date1.split('/')[2], date1.split('/')[1], date1.split('/')[0]),
      new Date(date2.split('/')[2], date2.split('/')[1], date2.split('/')[0]),
...
function filterColumn(val) {
  ...
  console.log(
    'Donnees :',
    this.data.filter(
      (row) => String(row[Object.keys(this.filter_data)[0]]).indexOf(val) != -1,
    ),
  );
...
async function updateDatas() {
  ...
  console.log(
    'Table result info avant parking :',
    this.data.filter((o) => o.typeacces == 'PARKING'),
  );
  ...
  const maxCols = countkeys.findIndex((item) => Number(item) == Number(maxlen));
  ...
  const minCols = countkeys.findIndex((item) => Number(item) == Number(minlen));
  ...
  console.log('Columns :', cols);
  console.log(
    'Table result info entre parking :',
    tmpdatas.filter((o) => o.typeacces == 'PARKING'),
  );
  ...
  console.log('INFOS Formulaire associé Demande Parking :', tmpFormDatas);
  /**
   * filtre lieu d'accès
   */
  ...
  //  transform ['nom','prenom'] en { nom:'',prenom:''}
  this.editRow = cols.reduce((column, key) => ((column[key] = ''), column), {});
    ...
    if (tmpFormDatas && tmpFormDatas.length == 1 && formFields) {
      ...
      formFields.forEach((val, index) => {
        if (this.newColumns.findIndex((o) => o.name == val.id) != -1) {
          let i = this.newColumns.findIndex((o) => o.name == val.id);
          this.newColumns[i].label = String(val.label).trim().replace(':', '');
...
onBeforeMount(() => {
  if (props.dbname && props.tablename) {
    console.log(
      'Query :',
      Object.entries(JSON.parse(props.query)).map(([key, val]) => ({
        key: key,
        val: val,
      })),
    );
...
onMounted(() => {
  if (this.dbname && this.tablename) {
    console.log(
      'Query :',
      Object.entries(JSON.parse(this.query)).map(([key, val]) => ({
        key: key,
        val: val,
      })),
    );
